module.exports = function (mpcp) {
  ...
  return {
    ...
    init: function (location) {
      // set the location to the last saved location, else set the server's default
    ...
    updateLocal: function (callback) {
      ...
      // this.local* always has a length of 1, but may have an empty object.
      // This fixes removal of "Empty directory"
      ...
      var start = 0
      var end = this.localFolders.length
      var html = ''
      ...
      for (i = current; i < this.localFolders.length; ++i) {
        if (current > end || current > mpcp.downloader.localFolders.length) { break }
    ...
    initEvents: function () {
      ...
      document.getElementById('downloader-url').addEventListener('keyup', e => {
        if (e.keyCode === 13) { mpcp.downloader.downloadFromDownloader() }
      })
