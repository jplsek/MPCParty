module.exports = function (mpcp) {
  ...
  return {
    ...
    updateLocal: function (callback) {
      ...
      var start = (mpcp.pages.currentPlaylist - 1) * mpcp.pages.maxPlaylist
      var end = ((mpcp.pages.currentPlaylist - 1) * mpcp.pages.maxPlaylist) + mpcp.pages.maxPlaylist
      var i
      var value
      ...
      for (i = 0; i < this.local.length; ++i) {
        ...
        if (mpcp.settings.pulse && ~this.toPulse.indexOf(value.Id)) {
    ...
    fromSortableSender: function (el, newIndex) {
      ...
      if (newIndex === 1 && document.getElementById(this.tbodyid).children[0].classList.contains('rem')) { newIndex = 0 }
      ...
      if (mpcp.browser.selected.length) {
        ...
        return
      } else if (el.classList.contains('artist') && mpcp.libraryArtists.selected.length) {
        mpcp.library.addMulti(mpcp.libraryArtists, newIndex, true, true)
        return
      } else if (el.classList.contains('album') && mpcp.libraryAlbums.selected.length) {
      ...
      if (this.selected.length) {
        $(this.selected).each(function (item, tr) {
          ...
          if (pos > newIndex) {
            ...
            // else if original location is about the "move to" location
            index = newIndex
            // console.log('dragged playlist: ' + file + ' to ' + index)
            komponist.moveid(file, index, function (err) {
              if (err) console.log(err)
          ...
          $(files).each(function (item, value) {
            ...
            if (value.file) {
              mpcp.playlist.addid(value.file, to++, function () {
                if (++j === Object.keys(files).length) { mpcp.playlist.addCallbackUpdate(callback) }
              })
            } else {
              if (++j === Object.keys(files).length) { mpcp.playlist.addCallbackUpdate(callback) }
            }
    ...
    addSong: function (file, to, dontScroll, callback) {
      ...
      if (!dontScroll) {
        ...
        } else if (mpcp.player.current && to === mpcp.player.current.Pos + 1) {
          this.addCallbackUpdate(function () {
            mpcp.playlist.goToCurrent()
          })
        } else {
    ...
    playSong: function (file) {
      ...
      komponist.playid(file, function (err, val) {
        ...
        if (err && err.message !== 'Integer expected: undefined [2@0] {playid}') {
        ...
        if (duplicate[file.file] && (mpcp.player.current === null ||
          ...
          komponist.deleteid(file.Id, function (err, val) {
            ...
            if (++j === Object.keys(mpcp.playlist.local).length &&
              callback) { mpcp.playlist.addCallbackUpdate(callback) }
          })
        } else {
          duplicate[file.file] = true

          if (++j === Object.keys(mpcp.playlist.local).length && callback) { mpcp.playlist.addCallbackUpdate(callback) }
        }
      })
    },
    ...
    moveToCurrent: function (file) {
      ...
      if (mpcp.playlist.selected.length) {
        ...
        $(mpcp.playlist.selected).each(function (item, tr) {
          ...
          } else if ((mpcp.player.current && mpcp.player.current.Pos > pos) || mpcp.player.current) {
    ...
    setPriorityFromForm: () => {
      if (!document.getElementById('priority-form').checkValidity()) {
        ...
        const fileid = document.getElementById('priority-form').dataset.fileid
