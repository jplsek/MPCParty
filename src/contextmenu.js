module.exports = function (mpcp) {
  ...
  $.contextMenu({
    ...
    build: function ($trigger, e) {
      ...
      if (table[0].classList.contains('song-list')) { title = $trigger.children('td:nth-child(2)').attr('title') }
      ...
      if (mpcp.pe.current && table.attr('id') !== 'playlist-song-list') {
        ...
        if (!($($trigger)[0].classList.contains('directory') ||
          ...
          $($trigger)[0].classList.contains('album'))) { items.infoBrowser = { name: 'Song information' } }
      } else {
        items = {
