  ...
  return {
    ...
    search: function (name, poppedState, callback) {
      ...
      function callbackSearch (files) {
        ...
        if (!files.length) {
          // note: when the search is nothing, it does not save to history
          html = '<tr class="directory gen"><td colspan="6">' +
    ...
    updatePosition: function (callback) {
      ...
      tr = $('.song-list tbody').children('.directory')
    ...
    updateLocal: function (callback) {
      ...
      var start = 0
      var end = this.localFolders.length + this.localFiles.length
      var html = ''
      ...
      for (i = current; i < this.localFolders.length; ++i) {
        if (current > end || current > mpcp.browser.localFolders.length) { break }
      ...
      for (; i < this.localFiles.length; ++i) {
        if (current > end || current - mpcp.browser.localFolders.length >
          mpcp.browser.localFiles.length) { break }
    ...
    addMulti: function (to, callback, dragging) {
      ...
      var i; var tr; var dir; var file; var j = 0

      function addFile (file) {
        mpcp.playlist.addSong(file, to, dontScroll, function () {
          if (++j === mpcp.browser.selected.length && callback) { callback() }
        })
      }
      ...
      if (mpcp.pe.current && !dragging) {
        ...
        if (to && mpcp.player.current && to !== mpcp.player.current.Pos + 1) { dontScroll = true }
        ...
        for (i = 0; i < this.selected.length; ++i) {
          ...
          if (tr.classList.contains('file')) {
            ...
            if (++j === this.selected.length && callback) { callback() }
          }
        }
      }
          ...
          var updateInterval = setInterval(function () {
            ...
            komponist.status(function (err, status) {
              ...
              if (status.updating_db === undefined) {
                // stop interval and send update-browser to everyone
                clearInterval(updateInterval)
                mpcp.lazyToast.info(
