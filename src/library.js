module.exports = function (mpcp) {
  // the library (alternative to file browser) controls the main #library
  return {
    hidden: true,
    ...
    addToHistory: function () {
      ...
      var albumHistory = ''
      var artistHistory = encodeURIComponent(mpcp.library.artist)
    ...
    getSongsFromAlbum: function (artist, album, callback) {
      ...
      function setSongs (err, files) {
        ...
        if (!files.length || (files.length === 1 && !files[0].Album && !files[0].Artist)) {
    ...
    addExternal: function (obj, artist, album, to, dontScroll) {
      if (obj.selected.length) {
        ...
        this.getSongsFromAlbum(artist, album, function (files) {
          for (var i = 0; i < files.length; ++i) { mpcp.playlist.addSong(files[i].file, to) }
        })
      }
    },
