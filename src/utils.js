module.exports = function (mpcp) {
  ...
  return {
    ...
    toFriendlyDDHHMM: function (str) {
      ...
      var hours = Math.floor(secNum / 3600) - (days * 24)
      var minutes = Math.floor(secNum / 60) - (days * 24 + hours) * 60
    ...
    getSimpleTitle: function (title, artist, file, playlistVal) {
      ...
      var fileStripped = mpcp.utils.stripSlash(file)
    ...
    toArray: function (obj) {
      if (!Array.isArray(obj)) {
        ...
        if (obj.length === 1 && $.isEmptyObject(obj[0])) { return [] }
    ...
    createSearch: function (input, callSearch, callReset, time) {
      ...
      $(input).focus(function () {
        // makes the instant search not as instant (instead of relying on every keyUp)
    ...
    // hides table rows as the user searches (input text box, table, data-*, clear input button)
